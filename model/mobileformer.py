    ...
    def __init__(self, in_channel, expand_size, out_channel, token_demension, kernel_size=3, stride=1, k=2):
        ...
        if stride == 2:
            self.strided_conv = nn.Sequential(
                nn.Conv2d(self.in_channel, self.expand_size, kernel_size=3, stride=2, padding=int(self.kernel_size // 2), groups=self.in_channel),
            ...
            self.conv2 = nn.Conv2d(self.in_channel, self.expand_size, kernel_size=3, stride=1, padding=1, groups=self.in_channel)
            ...
            self.conv2 = nn.Conv2d(self.expand_size, self.expand_size, kernel_size=3, stride=1, padding=1, groups=self.expand_size)
            self.bn2 = nn.BatchNorm2d(self.expand_size)
            self.ac2 = DynamicReLU(self.expand_size, self.token_demension, k=self.k)          
            self.conv3 = nn.Conv2d(self.expand_size, self.out_channel, kernel_size=1, stride=1)
            self.bn3 = nn.BatchNorm2d(self.out_channel)
...
class MobileFormer(nn.Module):
    ...
    def __init__(self, expand_sizes=[[96, 96], [144, 192], [288, 384, 576, 768]],
                       out_channels=[[24, 24], [48, 48], [96, 96, 128, 128]], 
                       num_token=4, d_model=128, in_channel=3, bneck_exp=32, bneck_out=16, 
                       stem_out_channel=16):
