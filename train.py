...
from dtset.videomatte import (
    VideoMatteDataset,
    VideoMatteTrainAugmentation,
    VideoMatteValidAugmentation,
)
from dtset.imagematte import (
    ImageMatteDataset,
    ImageMatteAugmentation
)
from dtset.coco import (
    CocoPanopticDataset,
    CocoPanopticTrainAugmentation,
)
from dtset.spd import (
    SuperviselyPersonDataset
)
from dtset.youtubevis import (
    YouTubeVISDataset,
    YouTubeVISAugmentation
)
from dtset.augmentation import (
    TrainFrameSampler,
    ValidFrameSampler
)
...
class Trainer:
    ...
    def init_model(self):
        ...
        # 查看自己网络参数各层名称、数值
            # print(name)
        # Freeze for hr training
        # freeze = ['backbone', 'decoder', 'project_mat', 'project_seg']  # parameter names to freeze (full or partial)
        # if any(freeze):
            # for k, v in self.model.named_parameters():
                # if any(x in k for x in freeze):
                    # self.log('freezing %s' % k)
